...
export function CourseCard({ course }: CourseCardProps) {
  ...
  const coverSrc = course.coverImage
    ? `${BASE_PATH}${course.coverImage}`
    : `${BASE_PATH}/images/courses/${course.courseId}.png`;

  return (
    <motion.div variants={fadeInUp}>
      <Link
        href={`/learn/${course.courseId}`}
        className="group flex h-full flex-col rounded-2xl bg-muted/50 overflow-hidden transition-colors hover:bg-muted"
      >
        {/* Cover Image */}
        ...
        <div className="flex flex-col flex-1 p-5">
          ...
          <h3 className="mt-3 text-base font-medium leading-snug group-hover:underline group-hover:underline-offset-4">
            {course.title}
          </h3>
