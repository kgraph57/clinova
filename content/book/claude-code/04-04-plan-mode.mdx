...
**「コードを書く前に設計する」。当たり前のことだが、AIの速さに流されると忘れがちである。Plan Modeは、この「設計ファースト」を仕組みで担保する。**
...
**Plan Mode**: Claude Codeは**読み取り系のツールしか使えない**。Glob、Grep、Readでコードベースを探索し、実装計画を立てる。ファイルの編集やコマンドの実行はできない。

この制約が重要である。計画段階で「うっかり実装を始めてしまう」ことを防ぎ、**設計と実装を明確に分離**する。
...
このように、Plan Modeは**双方向の設計プロセス**を実現する。
