...
description: "スラッシュコマンドで発動する専門スキル。/commit、/tdd、/code-reviewなど、よく使うワークフローをコマンド化する方法を解説する。"
slug: "03-02-skills"
order: 9
partId: "part03"
partTitle: "カスタマイズ"
...
**「/commit」と打つだけでコミットメッセージが自動生成される。「/tdd」と打つだけでテスト駆動開発のワークフローが始まる。Skillsは、あなたの開発パターンをコマンド化する仕組みである。**
...
これらの**スラッシュコマンド**は、「Skill」と呼ばれるプロンプトテンプレートによって実現されている。

Skillは、よく使う作業パターンを**再現可能なコマンド**にまとめたものだ。毎回同じ指示を手で入力する代わりに、一度Skillとして定義しておけば、スラッシュコマンド一つで発動する。
...
**フロントマター**でスキルのメタデータ（名前、説明、トリガー）を定義し、**本文**で具体的な指示を記述する。
...
スキルを一つずつ作る代わりに、**プラグイン**として複数のスキルをまとめてインストールすることもできる。
