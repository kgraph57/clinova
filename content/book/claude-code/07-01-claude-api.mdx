...
**Claude Codeの裏側で動いているのがClaude APIである。このAPIを直接使えば、自分だけのAIツールを構築できる。**
...
APIを直接使うことで、Claude Codeでは実現できない**カスタムワークフロー**を構築できる。
...
Claude APIの真髄は**ツール使用**にある。Claude Codeのツールシステムも、この機能で実現されている。
